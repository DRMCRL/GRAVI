...
cat("Trying to load SimpleUpset...\n")
library(SimpleUpset)
sessionInfo()
...
individual_peaks %>% 
  ...
  plotOverlaps(
    ...
    min_size = 10, n_intersections = 40, keep_empty_groups = TRUE
  ) +
  theme(
    panel.grid = element_blank(), 
    panel.border = element_rect(colour = "grey20", fill =  NA),
...
A set of treatment-specific `r target` peaks (i.e. *treatment peaks*) was defined for each condition by comparing the peaks detected when merging all treatment-specific samples, against those detected within each replicate.
Replicates which failed the previous QC steps are omitted from this step.
Treatment peaks which overlapped a peak in more than `r percent(config$peaks$qc$min_prop_reps)` of the *individual replicates passing QC* in each treatment group, were retained.
...
In addition to the treatment peaks, a set of `r comma(length(union_peaks))` treatment-agnostic `r target` **union peaks** were defined.
Union ranges were the *union* of all overlapping ranges defined and retained in one or more sets of treatment peaks.
...
`r if(length(vd) <= 3) glue("![*Number of {target} union peaks which overlap treatment peaks defined within each condition.*](assets/{target}/{target}_common_peaks.{fig_type})")`

```{r plot-common-upset, eval = length(vd) > 3, echo = length(vd) > 3, fig.height = 6, fig.cap = glue("*Number of {target} union peaks which overlap the treatment peaks defined within each condition.*")}
ql <- names(treatment_peaks) %>% 
  lapply(
...
treatment_peaks %>% 
  ...
  as_tibble() %>% 
  upset(
    intersect = names(treatment_peaks),
    base_annotations = list(
      `Peaks in Intersection` = intersection_size(
        text_mapping = aes(label = comma(!!size)),
        bar_number_threshold = 1, text_colors = "black", 
...
```{r plot-union-dist-to-tss-hist, fig.height=6, fig.cap = glue("*Distances from the centre of the {target} union peak to the transcription start-site shown as A) a histogram, and B) as a cumulative distribution. The vertical dashed line indicates the range considered to be a promoter during annotation preparation. {comma(sum(overlapsAny(union_peaks, tss)))} of the {comma(length(union_peaks))} {target} union peaks ({percent(mean(overlapsAny(union_peaks, tss)), 0.1)}) directly overlapped a TSS.*")}
a <- union_peaks %>% 
  join_nearest(
...
```{r plot-region-overlap, fig.height=8, fig.width=8, fig.cap = glue("*Proportions of {target} union peaks which overlap gene-centric features.*")}
union_peaks %>% 
  plotPie(
...
```{r plot-feature-overlap, eval = has_features, echo = has_features, fig.height=8, fig.width=8, fig.cap = glue("*The total number of {target} union peaks which overlap external features provided in {basename(config$external$features)}. If peaks map to multiple features, they are assigned to the one with the largest overlap.*")}
union_peaks %>% 
  plotPie(
...
```{r plot-feature-region-overlap, eval = has_features, echo = has_features, fig.cap = glue("*The total number of {target} union peaks overlapping external features and gene-centric regions. If a peak overlaps multiple features or regions, it is assigned to be the one with the largest overlap. Any peaks which don't overlap a feature have been excluded.*")}
union_peaks %>% 
  plotSplitDonut(
...
These are the most highly ranked Union Peak (by `score`) and any treatment peaks which are unique to each treatment group, after excluding the most highly ranked union peak.
