...
saveRDS(fig_path, "debug_figpath.rds")
...
saveRDS(db_results, "debug_db_results_pairwise.Rmd")
db_results %>% 
  plotPairwise(
    var = AveExpr, # custom line, explicitly state col to plot
    #var = cpm_column, # og line, also yields "AveExpr" but doesn't work for some reason
    alpha = 0.6, side_panel_width = 0.2,
  ) +
  scale_fill_manual(
    values = setNames(
...
saveRDS(profile_heatmaps, "debug_profile_heatmaps.rds") #testing purposes
saveRDS(grl_to_plot, "debug_grltoplot.rds") #testing purposes
timetest <- system.time(print(profile_heatmaps[[1]])) #testing purposes only
saveRDS(timetest, "debug_timetest.rds") # testing purposes only
htmltools::tagList(
  mclapply(
    ...
    mc.cores = 1 # new line to see if this stops this from taking forever
    #mc.cores = threads # og line
  )
)
```
...
msigdb <- msigdbr(species = extra_params$enrichment$species) %>% 
  rename(gs_cat = gs_source_species, gs_subcat = gs_subcollection) %>%  # added line to make gs_cat and gs_subcat recognisable
  dplyr::filter(
