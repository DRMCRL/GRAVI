...
library(fgsea)
library(metap)
# Thao added the following line to make a DE col in rnaseq containing logical vals for DEG (diff analysis originally done via DESEq2 which doesnt have this col, is this an edgeR thing?
rnaseq$DE <- rnaseq[, rna_fdr_col][[1]] < 0.05
# back to og script
rna_status <- rnaseq %>% 
  mutate(
    de_status = case_when(
      !!sym(rna_fdr_col) < 0.05 & !!sym(rna_lfc_col) < 0 ~ "Down", #0.05 replacing fdr_alpha
      !!sym(rna_fdr_col) < 0.05 & !!sym(rna_lfc_col) > 0 ~ "Up",#0.05 replacing fdr_alpha
      !!sym(rna_fdr_col) >= 0.05 ~ "Unchanged" #0.05 replacing fdr_alpha
    )
  ) %>% 
...
```{r any_diff_sites}
##TEST SCRIPT ONLY!!!
#unadjusted pval and threshold adjusted FOR TESTING ONLY to give a more manageable number of peaks to run on
# in actual run DO NOT USE THE FOLLOWING 2 LINES
#fdr_column <- "P.Value"
#fdr_alpha <- 0.05
...
cmn_genes <- intersect(
  rnaseq %>% dplyr::filter(!!sym(rna_fdr_col) < 0.05) %>% pull("gene_id"), #0.05 replacing fdr_alpha
  merged_results %>% 
