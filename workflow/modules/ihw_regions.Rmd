...
if (ihw_proceed)  fdr_column <- "fdr_ihw" #og script, blocked out to enforce pval for testing. do run this in actual run
...
Independent Hypothesis Weighting (IHW) [@IgnatiadisIHW2016] was then used to partition the raw p-values for `r target` differential binding by genomic regions defined previously, and estimate weights to apply to each p-value as a divisor.
...
lblr <- as_labeller(c(regions, "Other" = "Other"))
ihw %>%
  as.data.frame() %>% 
  mutate(
...
```{r plot-ihw-pvals, echo = TRUE, fig.height= 3 + 2 * ceiling(length(levels(merged_results$ihw_covariate)) / 3), fig.cap = "*P-Value distributions within all final data partitions. The size of each parition is given within each panel.*"}
y <- ggplot_build(
