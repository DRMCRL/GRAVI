...
#fdr_alpha <- 0.05 # FOR TESTING ONLY. do not use in actual run
fdr_alpha <- config$comparisons$fdr # OG code, use this in actual run
...
if (length(hic_path) > 0)
  if (file.exists(hic_path)) {
    ...
    reg_combs <- expand.grid(regions, regions) %>% 
      ...
      unique()
    hic$counts <- rep(1, length(hic)) # this is added since the original bedpe file has imposed score = 0 for all interactions and this throws error
...
saveRDS(merged_results, "debug_merged_results_first.rds")
```
...
```{r set-ihw}
#fdr_column <- "P.Value" # FOR TESTING ONLY. do not use in actual run
#fdr_alpha <- 0.05 # FORE TESTING ONLY.
fdr_column <- str_subset(names(mcols(merged_results)), "fdr|FDR") # actual code, use this in actual  run
...
saveRDS(merged_results, "debug_merged_results_postset.rds")
```
...
```{r map-to-regions}
saveRDS(merged_results, "debug_merged_results_postihw.rds")
merged_results <- merged_results %>% 
  mutate(
    region = bestOverlap(
      ., 
...
# TEST
saveRDS(gtf_gene, file = "debug_gtf_gene_preresults.rds") 
saveRDS(merged_results, file = "debug_merged_results_postmap.rds")
```
...
#plyranges::slice(seq_len(show_n))%>% # original line, replacement below
  head(show_n) %>% 
...
```{r grl-to-plot}
#fdr_column <- "P.Value" # TESTING ONLY. remove this in actual run
...
# the next chunk is tmeporarily inserted to see where the error in pivot_longer() comes from
tmp <- grl_to_plot %>% 
  lapply(
    function(x){
      filtered_counts %>% 
...
saveRDS(tmp, "debug_tmp.rds")

print(tmp)
print(names(tmp))
print(sapply(tmp, class))
...
# the below is the og script in full
#grl_to_plot %>% 
#  lapply(
#    function(x){
#      filtered_counts %>% 
...
# this bit is added since using the shorthand in the original script results in a nested granges object and not plottable. this will be applied for all plots of this kind in this section
gr <- grl_to_plot$top_down_by_expr
bad_cols <- vapply(mcols(gr), inherits, logical(1), "GRanges")
mcols(gr)[bad_cols] <- NULL
...
# see above
gr <- grl_to_plot$top_down_by_fdr
bad_cols <- vapply(mcols(gr), inherits, logical(1), "GRanges")
mcols(gr)[bad_cols] <- NULL
plotHFGC(gr,
...
#see above
gr <- grl_to_plot$top_down_by_lfc
bad_cols <- vapply(mcols(gr), inherits, logical(1), "GRanges")
mcols(gr)[bad_cols] <- NULL
plotHFGC(gr,
...
#see above
gr <- grl_to_plot$top_down_no_macs2
bad_cols <- vapply(mcols(gr), inherits, logical(1), "GRanges")
mcols(gr)[bad_cols] <- NULL
plotHFGC(gr,
...
#see above
gr <- grl_to_plot$top_up_no_macs2
bad_cols <- vapply(mcols(gr), inherits, logical(1), "GRanges")
mcols(gr)[bad_cols] <- NULL
plotHFGC(gr,
...
msigdb <- msigdbr(species = "Mus musculus") %>% #should have species as an adjustable param 
  rename(gs_cat = gs_source_species, gs_subcat = gs_subcollection) %>% 
  dplyr::filter(
...
saveRDS(gtf_gene, file = "debug_gtf_gene.rds") ## ADD ME NEXT TIME
saveRDS(merged_results, file = "debug_merged_results.rds") ## ADD ME NEXT TIME
# the next chunk is custom added to break down the leadup to nullp() (which runs into error) for diagnostics purpose
tmp1 <- gtf_gene %>%
  mutate(w = width(.)) %>%
  as_tibble() %>%
...
tmp2 <- tmp1 %>%
  mutate(mapped = ifelse( is.na(gene_id),FALSE,gene_id %in% unlist(merged_results$gene_id)))
tmp3 <- tmp2 %>%
  filter(!is.na(gene_id)) %>%
  filter(!is.na(w), w > 0) %>%
  mutate(bias = log10(w)) %>%
  filter(is.finite(bias))
mapped_vec <- tmp3$mapped
names(mapped_vec) <- tmp3$gene_id
bias_vec <- tmp3$bias
names(bias_vec) <- tmp3$gene_id
stopifnot(!any(is.na(names(mapped_vec))))
stopifnot(!any(duplicated(names(mapped_vec))))
saveRDS(tmp1, file = "debug_tmp1.rds")
saveRDS(tmp2, file = "debug_tmp2.rds")
saveRDS(tmp3, file = "debug_tmp3.rds")

pwf_mapped <- goseq::nullp(
  mapped_vec,
...
pwf_mapped <- gtf_gene %>% 
  ...
  arrange(desc(w)) %>% 
  distinct(gene_id, .keep_all = TRUE) %>% 
  # custom script to combat NA
  mutate(mapped = ifelse( is.na(gene_id),FALSE,gene_id %in% unlist(merged_results$gene_id))) %>%
 # mutate(mapped = gene_id %in% unlist(merged_results$gene_id)) %>% #original script
  with(
    nullp(
