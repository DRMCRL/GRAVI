...
### this chunk is modified from OG to avoid error caused by having no features file
...
test_regions <- if (has_features) c(regions, features) else regions
#the below is the original block
#features <- read_rds(all_input$features) %>%
 # endoapply(select, feature) %>%
 # unlist() %>%
 # names_to_column("source") %>%
...
if (length(peaks) < 100){ #100 as dummy value, since regioner_params$min_regions doesn't exist as a params
