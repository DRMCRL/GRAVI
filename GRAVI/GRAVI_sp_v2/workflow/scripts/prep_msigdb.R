...
msigdb <- msigdbr(db_species = msigdb_params$db_species, species = msigdb_params$species) %>%
#msigdb <- msigdbr(msigdb_params$species) %>% # blocked out since this is not sufficient for mouse
  dplyr::filter(
