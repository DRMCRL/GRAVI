...
# theres a pander line in this section that has been removed. hashing out the line didn't remove its readthrough, which leads to error if config$external is empty. original line can be found in Stevie's original script.
## Genome Annotations
...
msigdb %>% 
  ...
  summarise(`Gene Sets` = dplyr::n(), .by = ends_with("collection")) %>% 
...
`r if (nrow(dplyr::filter(tf_db, cluster %in% top3[[1]])) >= 20) "The largest cluster was too big to visualise all motifs"`

```{r tf-cl1, eval = nrow(dplyr::filter(tf_db, cluster %in% top3[[1]])) < 20, fig.cap = glue("*Information Content logos for all TFBMs within the largest cluster. Reverse complement sequences from the defined matrix are annotated with RC.*")}
tf_db %>% 
...
`r if (nrow(dplyr::filter(tf_db, cluster %in% top3[[2]])) >= 20) "The second largest cluster was too big to visualise all motifs"`

```{r tf-cl2, eval = nrow(dplyr::filter(tf_db, cluster %in% top3[[2]])) < 20,  fig.cap = glue("*Information Content logos for all TFBMs within the second largest cluster. Reverse complement sequences from the defined matrix are annotated with RC.*")}
tf_db %>% 
...
`r if (nrow(dplyr::filter(tf_db, cluster %in% top3[[3]])) >= 20) "The third largest cluster was too big to visualise all motifs"`

```{r tf-cl3, eval = nrow(dplyr::filter(tf_db, cluster %in% top3[[3]])) < 20, fig.cap = glue("*Information Content logos for all TFBMs within the third largest cluster. Reverse complement sequences from the defined matrix are annotated with RC.*")}
tf_db %>% 
