...
motif_params <- here::here("config", "json", "motif_analysis_param.json") %>%
  jsonlite::fromJSON() %>%
  pluck(target)
peak_calling_params <- here::here("config", "json", "peak_calling_param.json") %>%
  jsonlite::fromJSON() %>%
  pluck(target)
diff_signal_params <- here::here("config", "json", "differential_signal_param.json") %>%
  jsonlite::fromJSON() %>%
  pluck(target)
...
if (!is.null(diff_signal_params$contrasts)) {
  ## Ensure levels respect those provided in contrasts
  treat_levels <- diff_signal_params$contrasts |> 
    as.character() |> 
    unique()
}
replicate_col <- setdiff(
...
samples <- samples %>%
  ...
  arrange(treat, !!sym(replicate_col))
...
## Gene Annotations
gtf <- read_rds(file.path(all_paths$annotation, "gtf.rds"))
...
The strategy for identification of genomic regions with detectable `r target` signal (i.e. peaks), was to run `macs2 callpeak` [@Zhang18798982] on each individual sample, as well as on all samples within each treatment group (`r pander(treat_levels)`).
The settings for `macs2 callpeak` were:

`r pander(peak_calling_params[c("gsize", "fdr", "keep_duplicates", "macs2_extra", "peak_type")])`

Additional QC and peak merging settings included:

`r pander(peak_calling_params[c("merge_fdr", "merge_within", "min_prop_reps", "outlier_threshold", "allow_zero")])`

Bedgraph files were produced for each sample to return values scaled using the `--SPMR` flag, as signal per million reads, which is analogous to the common Counts Per Million (CPM) value.
Where required, these were converted to BigWig files in order to save storage space & speed up analysis.

### Identification of Peaks and QC Description

#### Individual Sample QC settings

For individual samples, any sample with > `r peak_calling_params$outlier_threshold`x the median number of peaks found across all samples within that treatment group would be marked as failing QC and excluded from further analysis.
Any samples with no detected peaks would also be `r ifelse(peak_calling_params$allow_zero, 'retained, to account for any cytoplasmic-nuclear shift in response to treatment', 'excluded')`.

#### Filtered Peaks

A set of *treatment-specific* filtered peaks was defined for each treatment group, based on the peaks identified when running `macs2 callpeak` on all samples within a treatment group.
Any peak which was not found in >`r percent(peak_calling_params$min_prop_reps)` of *individual samples* within that treatment group was then discarded.
Peaks within `r peak_calling_params$merge_within`bp of each other were merged into a single peak.
Peaks detected within merged samples were only retained with an FDR < `r peak_calling_params$merge_fdr`

Peaks which overlapped any blacklisted or grey-listed regions were discarded.

#### Consensus Peaks

Consensus peaks across all treatment groups were then produced by combining all sets of filtered (i.e. *treatment-specific*) peaks.
These peaks were defined as the union of all peaks across all treatment groups.
Overlapping peaks between treatment groups were merged to create a peak encompassing the range identified in all treatment groups.

### Additional Analysis

After identification of peaks

- A selection of highly-ranked peaks were plotted
- The relationship to detected genes within any transcriptomic data was assessed
- Enrichment of binding sites within gene-sets was assessed
- The relationship of binding sites to genomic regions and any features was assessed
- Motif analysis was performed, checking for general motif enrichment as well as any association between motifs and peak centres (i.e. positional bias)

## QC {.tabset}

This section provides a simple series of visualisations to enable detection of any problematic samples.

- Library Sizes: These are the total number of alignments contained in each `bam` file, as passed to `macs2 callpeak`
...
macs2_logs %>%
  ...
  pander(
    ...
    caption = glue(
      ...
      "threshold of {percent(peak_calling_params$fdr)} during peak calling.",
      ifelse(
        any_fail,
        glue(
          "Samples shown in italics were marked for exclusion from ",
          "downstream analysis as they identified a number of peaks beyond ",
          "+/- {peak_calling_params$outlier_threshold}-fold the median number of ",
...
```{r plot-macs2-peaks-individual, fig.height = 0.75 * fh, fig.cap = glue("*Peaks identified for each {target} sample. The number of peaks passing the inclusion criteria for `macs2 callpeak` (FDR < {peak_calling_params$merge_fdr}) are provided. Any samples marked for exclusion are coloured as indicated in the figure legend.*")}
suppressWarnings(
  macs2_logs %>%
...
valid_sets <- makeConsensus(individual_peaks) %>% 
  ...
  pull(samples) %>% 
  setNames(map_chr(., \(x) names(which(sets == x)))) %>% 
  lapply(function(x) str_split(x, "; ")[[1]])
lb <- setNames(samples$label, samples$sample)
individual_peaks %>%
  setNames(lb[names(.)]) %>% 
  ...
  mutate(
    ...
    highlight = case_when(
      !!!imap(
        valid_sets, 
        \(cols, name) (if_all(all_of(cols)) & !if_any(all_of(setdiff(lb, cols)))) ~ name
      )
    )
  ) %>% 
  simpleUpSet(
    sets = unname(lb), n_intersect = 20, sort_sets = NULL, min_size = 10,
    set_layers = default_set_layers(
      fill = "set", expand = c(0.3, 0),
      scale_fill_manual(
        values = colours$treat[as.character(samples$treat)] |> setNames(lb)
      ),
      guides(fill = guide_none()),
      name = "Peaks Overlapping Consensus"
    ),
    intersect_layers = default_intersect_layers(
      fill = "highlight", name = "Peaks in Intersection\n", expand = c(0, 0.1),
      scale_fill_manual(
        values = c(all = "darkorange", colours$treat), na.value = "grey23"
      ),
      guides(fill = guide_none())
    ),
    grid_layers = default_grid_layers(
      colour = "highlight", name = "Intersection",
      scale_colour_manual(
        values = c(all = "darkorange", colours$treat), na.value = "grey23"
      )
    ),
    annotations = list(
      geom_boxplot(aes(y = qValue), outliers = FALSE),
      scale_y_continuous(name = "Median qValue\n")
    )
  )
```
  ...
  names(ranges_by_rep_treat) %>% 
    lapply(
      function(i) {
        ...
        if (length(x) == 1) {
          fig_fun(fig_out, width = fw, height = fh)
          grid.newpage()
        ...
        if (length(x) == 2) {
          ...
          a2 <- length(x[[2]]) - a12
          args <- glue(
            here::here("scripts", "plot_venn.py "),
            "-a1 {a1} -a2 {a2} -a12 {a12} ",
            ...
            upset(
              ...
              set_sizes = (
                upset_set_size() +
                  ...
                  ylab(glue("Macs2 Peaks (FDR < {peak_calling_params$fdr})")) +
                  theme(
                    text = element_text(size = 14),
                    panel.grid = element_blank(), 
                    axis.line = element_line(colour = "grey20"),
...
Peaks detected when merging samples, and which overlapped a peak in more than `r percent(peak_calling_params$min_prop_reps)` of the *individual replicates passing QC* in each treatment group, were retained.
...
treat_levels %>% 
  ...
  pander(
    ...
    caption = glue(
      ...
      "{percent(peak_calling_params$min_prop_reps)} of the qc-passing samples for each treatment group,",
      "as described above.",
      .sep = " "
    )
  )
...
Resulting values for the `score` were calculated as the *maximum* across all treatments.
*Peak summits* were taken as the weighted mean position of all summits which comprise the union peak, using the score as weights.

#### `r ifelse(length(treat_levels) < 3, "Venn Diagram", "Upset Plot")`

```{r plot-common-peaks, fig.width = ifelse(length(treat_levels) < 3, 0.7 * fw, fw), fig.height = 0.7 * fw, fig.cap = glue("*Number of {target} consensus peaks which overlap the filtered peaks defined within each condition.*")}
plotOverlaps(
  filtered_peaks, exp_sets = 1/3,
...
```{r plot-peaks-by-feature, echo = has_features, fig.height = ifelse(length(feat_names) < 3, fh / 2, fh), fig.cap = glue("*{target} peaks which overlap each provided feature, coloured by gene-centric regions defined previously. Features may overlap with each other, and as such, peaks may overlap multiple feature types and be included within multiple categories. Total overlapping peaks for each feature type are shown in bold, wth percentages being relative to the total number of consensus peaks ({comma(length(consensus_peaks))}). Individual segments are labelled if representing >= 3% of the total number of peaks. Features with fewer than 1% of peaks aligning to them are collapsed into the category 'Other'*")}
if (has_features) {
  consensus_peaks %>% 
    as_tibble() %>% 
    ...
    geom_label(
      ...
      data = . %>% 
        dplyr::filter(n > 0.03 * length(consensus_peaks)) %>% 
        mutate(
          label = glue("{comma(n)}\n{percent(n / length(consensus_peaks), 0.1)}")
        ),
      alpha = 0.6, size = 3
    ) +
    ...
    labs(
      x = glue("Total Overlapping {target} Peaks"), 
...
for (i in treat_levels) {
  grl_to_plot[[i]] <- filtered_peaks[[i]] %>% 
    filter_by_non_overlaps(filtered_peaks[names(filtered_peaks) != i]) %>% 
    subset(score == max(score)) %>% 
    granges() %>% 
    unname() %>% 
    .[1]
...
These are the most highly ranked Consensus Peak (by `score`) and the most highly ranked peak for each of `r pander(treat_levels)` which was unique to that set of filtered peaks, without being identified in any other treatment group.
...
htmltools::tagList(
  lapply(
    ...
    function(i) {
      ...
      ## Define the caption
      gr <- join_nearest(grl_to_plot[[i]], gtf$gene, distance = TRUE)
      d <- gr$distance
      gn <- gr$gene_name
      peak_desc <- ifelse(
        nm == "union",
      ...
      htmltools::div(
        htmltools::div(
          ...
          htmltools::div(
            ...
            htmltools::img(src = fig_link, width = 960),
            htmltools::p(
              class = "caption", htmltools::tags$caption(cp)
            )
          )
        )
      )
...
### Low Ranked Peaks {.tabset}

```{r select-low-ranked}
grl_to_plot <- GRangesList()
grl_to_plot$consensus <- consensus_peaks %>% 
  subset(score == min(score)) %>% 
  granges() %>% 
  .[sample.int(length(.), 1)]
for (i in treat_levels) {
  grl_to_plot[[i]] <- filtered_peaks[[i]] %>% 
...
As a counter to the highly ranked peaks, coverage for a small set of *low ranking peaks* are shown below as representative of the low confidence calls.
These are the most low-ranked consensus peak (by `score`) and the most low-ranked peak for each of `r pander(treat_levels)` which was unique to that set of filtered peaks, without being identified in any other treatment group.
These effectively represent the *low-probability peak calls*, and it may be assumed that, from a quality assessment perspective, the majority of peaks will lie **somewhere between the highly-ranked and low-ranked peaks**.
    ...
    pander(
      ...
      caption = glue(
        ...
        *Relationship of RNA data to genes with peaks mapped to them.
        Fisher's Exact Test was performed to determine any association bewteen 
        a gene being detected in the transcriptomic data, and a {target} peak 
        being mapped to that gene. Taking the reported p-value, any significant 
        association implies that {target} is likely
        to be playing a regulatory role within that dataset. A total of 
        {comma(length(gtf$gene))} genes are contained in the provided GTF.*
        "
      )
    )
}
...
saveRDS(consensus_peaks, "debug_consensus_peaks.rds")
if (has_rna) {
  tagList(
    names(rna) %>% 
      lapply(
        \(i) {
          ...
          no_peak <- tibble(
            ...
            total = gtf$gene$gene_id %>% 
              setdiff(unlist(consensus_peaks$gene_id)) %>% 
              unique() %>% 
              length(),
            detected = x$gene_id %>% 
              setdiff(unlist(consensus_peaks$gene_id)) %>% 
          ...
          tbl <- consensus_peaks %>% 
            ...
            { # this next chunk replaces the blocked out bit above to prevent a no-feature error
 		 if ("feature" %in% names(.)) {
    			mutate(.,source = str_extract(feature, ".+: ") %>%
        		str_remove_all(": ") %>%
        		str_replace_na(""),
      			feature = str_remove_all(feature, ".+: ")    )
		  } else {mutate(., source = "") }
		} %>% 
            dplyr::select(source, any_of("feature"), region, everything()) %>% #small mod to feature bit
            dplyr::select(where(\(x) !all(x == ""))) %>% 
            dplyr::filter(adj_p < 0.05) %>% 
            reactable(
              showPageSizeOptions = TRUE, sortable = TRUE,
              ...
              columns = list(
                ...
                total = colDef(
                  glue("Mapped to {target} Peak"), format = comma_col,
              ...
              columnGroups = list(
                colGroup("Gene Expression", str_subset(names(.), "(detected|prop)$"))
              ),
              elementId = tbl_id
            )
          cp <- htmltools::em(
            glue(
              "
              Genes mapped to {target} peaks broken down by region and any 
              ...
              association between a gene being detected, and having detectable 
              {target} signal within that region. An 
              additional {comma(no_peak$detected)} genes were detected in this
              dataset without any evidence of associated {target} signal.
          ...
          htmltools::div(
            ...
            tagList(
              tags$button(
                ...
                onclick = glue(
                  "Reactable.downloadDataCSV('{tbl_id}', '{target}_{tbl_id}.csv')"
                )
              ),
              tags$button(
                "Expand/Collapse All",
...
enrich_arg_list <- list(
  test_ranges = consensus_peaks, db = msigdb, gtf = gtf$gene, bg = NULL, 
...
saveRDS(enrich_arg_list, "debug_enrich_arg_list.rds")
...
```{r tbl-enrich-res, eval = n_enrich > 0, echo = FALSE}
tbl_id <- "enrich-res-table"
tbl <- enrich_df %>% 
  dplyr::filter(adj_p < enrich_params$alpha) %>% 
...
tl <- tagList(
  tags$button(
    tagList(fontawesome::fa("download"), "Download as CSV"),
    onclick = glue(
      "Reactable.downloadDataCSV('{tbl_id}', '{target}_enrich_res.csv')"
    )
  ),
  tags$button(
    "Expand/Collapse All",
...
```{r zscore-heatmap, fig.cap = glue("*Normalised Z-scores showing the effect of shifting the subset of changed {target} peaks within a $\\pm$ {slot(mlz, 'parameters')$window / 1e3}kb window around the existing peaks.*")}
shifted_z %>% 
  arrange(abs(shift), Z) %>% 
  mutate(
...
if (n_enrich > 1) {
  ...
  cor_mat <- motif_list %>% 
    ...
    to_list() %>% 
    getPwmMatches(
      min_score = motif_params$min_score, fw_seq, mc.cores = threads
    ) %>% 
    ...
    dplyr::select(seq, altname, detected) %>% 
    pivot_wider(
      names_from = "altname", values_from = "detected", values_fill = FALSE
    ) %>% 
    dplyr::select(-seq) %>% 
...
clust_names <- motif_pos_res %>% 
  ...
  summarise(
    cl_name = paste(unique(name), collapse = "/"),
    name = name[[1]], .by = cluster,
  ) %>% 
  mutate(cl_name = setNames(cl_name, name)) %>% 
  pull(cl_name)
```
...
top_matches <- motif_list %>% 
  ...
  to_list() %>% 
  getPwmMatches(
    fw_seq, min_score = motif_params$min_score, 
    abs = motif_params$abs, best_only = TRUE, mc.cores = threads
  ) %>% 
  keep_at(map_int(., nrow) > 0)
top_matches  %>%
  plotMatchPos(
...
```{r heatmap-pos, echo = n_pos > 0, fig.cap = glue("*The top-ranked motifs by positional bias showing the total number of matches within each bin. Motifs are clustered by similarity of totals within this figure when more than two are being displayed. A maximum of 25 motifs are shown in this figure.*")}
if (n_pos > 0) {
  top_pos <- motif_pos_res %>% 
    dplyr::filter(adj_p < motif_params$alpha) %>% 
    dplyr::slice(1:25) %>% 
...
out_path <- file.path("output", "results", "signal_summary", target)
if (!dir.exists(out_path)) dir.create(out_path, recursive = TRUE)
all_out <- list(
  enrichment = file.path(out_path, glue("{target}_enrichment.csv")),
  localz = file.path(out_path, glue("{target}_localz.csv")),
  renv = file.path(
    "output", "envs", glue("{target}_signal_summary.RData")
  )
)
enrich_df %>% 
  mutate(gene_id = map_chr(gene_id, paste, collapse = "; ")) %>% 
  write_csv(here::here(all_out$enrichment))
write_csv(localz_df, here::here(all_out$localz))
...
```{r existing_in, echo = FALSE}
existing_in <- list(
  motif_pos_res = file.path(
    all_paths$peaks, "{target}", "{target}_motif_position.tsv.gz"
  ),
  motif_enrich_res = file.path(
    all_paths$peaks, "{target}", "{target}_motif_enrichment.tsv.gz"
  )
)
```
