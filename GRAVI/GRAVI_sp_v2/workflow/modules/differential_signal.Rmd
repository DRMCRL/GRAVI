...
cp <- htmltools::em(
  glue(
    ...
    {treat_levels[[2]]} against the baseline signal observed in {treat_levels[[1]]}.
    "
  )
)
tl <- tagList(htmltools::tags$caption(cp), tbl)
...
tbl <- df %>% 
  reactable(
    ...
    columns = list2(
      range = colDef(
        "Range", minWidth = 10 * fs,
        cell = \(value) str_replace_all(value, ":", ": ")
      ),
      w = colDef(
        "Width (bp)", maxWidth = 5 * fs,
        style = \(value) {
...
tl <- tagList(htmltools::tags$caption(cp), buttons, tbl)
htmltools::browsable(tl)
```
...
if (n_sig > 0) {
  results %>% 
    ...
    plotSplitDonut(
      ...
      label_alpha = 0, inner_label_colour = "white"
    )
}
```
...
#cb <- config$genome$build %>% str_to_lower() %>% paste0(".cytobands") 
#data(list = cb)
#bands_df <- get(cb)
#stopifnot(colnames(bands_df) == c("chrom", "chromStart", "chromEnd", "name", "gieStain"))
## The coverage
bwfl <- list2(
  "{target}" := file.path(
...
if (n_sig > 0) {
  ...
  saveRDS(enrich_args,"debug_enrich_args.rds")
...
increased_enrich_df <- tibble(gene_id = character())
...
```{r enrich-decreased, echo = n_decreased > 0 & n_decreased != n_sig}
decreased_enrich_df <- tibble(gene_id = character())
if (n_decreased > 0 & n_decreased != n_sig) {
  enrich_args <- list(
...
desc <- glue(
  "When testing for enrichment in the {comma(n_decreased)} sites ({percent(n_decreased / n_ranges, 0.1)}) showing evidence of any *decreased* signal, { comma(n_decreased_enrich)} pathways were considered to be enriched using an adjusted p-value < {enrich_params$alpha} as the criteria for considering a pathway as enriched."
)
```
...
saveRDS(fw_seq, "debug_fwseq.rds")
...
saveRDS(n_pos_res, "debug_nposres.rds")
```
...
if (n_enrich_res > 1) {
  ...
  cor_mat <- motif_db %>% 
    ...
    to_list() %>% 
    getPwmMatches(
      fw_seq$changed, min_score = motif_params$min_score, 
      mc.cores = threads
    ) %>% 
    ...
    distinct(name, seq) %>% 
    left_join(
      dplyr::select(motif_enrich_res, altname, name), by = "name"
    ) %>% 
    ...
    cor(method = "spearman")
saveRDS(motif_enrich_res, "debug_motif_enrich_res.rds")
saveRDS(top_enrich, "debug_motif_top_enrich.rds")
saveRDS(cor_mat, "debug_motif_cor_mat.rds")
...
saveRDS(top_pos, "debug_toppos.rds")
```
...
saveRDS(top_matches, "debug_topmatches.rds")
saveRDS(motif_db, "debug_motifdb.rds")
```
...
```{r define-outputs}
out_path <- file.path("output", "results", "differential_signal", target)
if (!dir.exists(out_path)) dir.create(here::here(out_path), recursive = TRUE)
all_out <- list(
  csv = file.path(out_path, glue("{comp}-differential_signal.csv")),
  ...
  localz = file.path(out_path, glue("{comp}-localz.csv")),
...
results %>% 
  ...
  write_csv(here::here(all_out$csv))
## Enrichment results need a bit of tweaking to match the tables
changed_enrich_df %>% 
  dplyr::select(-gene_id) %>% 
  write_csv(here::here(all_out$changed))
increased_enrich_df %>% 
  dplyr::select(-gene_id) %>% 
  write_csv(here::here(all_out$increased))
decreased_enrich_df %>% 
  dplyr::select(-gene_id) %>% 
  write_csv(here::here(all_out$decreased))
## LocalZ
localz_df %>% write.csv(here::here(all_out$localz))
