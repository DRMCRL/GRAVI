...
During preparation of the shared peaks, any regions which were smaller than 

## Peak Comparisons {.tabset}

Before the inspection of shared peaks, a summary of the individual targets is given below.
...
```{r plot-overlaps, fig.width = ifelse(n_targets > 2, fw, fh), fig.cap = glue("*Taking the combined overlap between any targets as the set of regions, the number of regions with each target overlapping these regions is shown. Consensus regions were taken as the union ranges, without considering coverage and may appear as different numbers to the shared peaks in the table above.*")}
plotType <- ifelse(n_targets > 2, "upset", "venn")
plotOverlaps(
  consensus_peaks, type = plotType, set_col = colours$target[1:2], exp_sets = 0.3
) # the [1:2] bit was added, as colours$target has 3 values and plotOverlaps() accepts only 2
```
...
```{r plot-shared-dist-to-tss-hist, fig.height = 0.75 * fh, fig.cap = glue("*Distances from the centre of each set of consensus peaks to the nearest transcription start-site shown as A) a histogram, and B) as a cumulative distribution. The vertical dashed line in B indicates the range considered to be a promoter during annotation preparation.*")}
df <- consensus_peaks %>% 
  endoapply(granges) %>% 
  lapply(
    \(x){
...
Given that `r n_targets` targets were included in the dataset, the common regions were defined as the `r comma(length(shared_peaks))` overlapping regions across *all targets*, but choosing the overlapping ranges where signal was detected in at least `r n_targets - 1` of the `r n_targets` targets.
This ensures any individual targets with broader peaks do not lead to unrealistically broad shared regions.

```{r tbl-shared}
tibble(
  `Shared Peaks` = length(shared_peaks),
  `Median Width` = median(width(shared_peaks)),
  Q25 = quantile(width(shared_peaks), 0.25),
  Q75 = quantile(width(shared_peaks), 0.75),
  `Smallest Peak` = min(width(shared_peaks)),
  `Broadest Peak` = max(width(shared_peaks))
) %>% 
  mutate(IQR = glue("[{comma(Q25)}, {comma(Q75)}]")) %>% 
  dplyr::select(-starts_with("Q")) %>% 
  pander(
    justify = "rrrrr", 
    caption = "Summary of shared peaks obtained across all targets"
  )
```
        ...
        \(i) {
          ...
          cp <- htmltools::em(
            glue(
              ...
              detected in the transcriptomic dataset {i}. Fisher's test was 
...
enrich_arg_list <- list(
  test_ranges = shared_peaks, db = msigdb, gtf = gtf$gene, bg = NULL, 
...
```{r enrich-network-genomic, echo = n_enrich_genomic >= min(network_params$size), eval = n_enrich_genomic >= min(network_params$size), fig.cap = glue("*The {n_enrich_genomic} most highly ranked gene sets from enrichment analysis when using the entire genome as the background. Edge thickness denotes similarity between connected nodes. Lighter node colours indicate increased significance. Label transparency and the outline thickness of each node also represent the proportion of each gene set mapped to a range with all targets detected. Node size indicates the number of genes mapped to a range. Networks are capped at a maximum of {max(network_params$size)} gene sets*")}
tg_enrich_genomic %>% 
...
bg <- consensus_peaks %>% 
  ...
  reduceMC()
enrich_arg_list <- list(
  test_ranges = shared_peaks, db = msigdb, gtf = gtf$gene, bg = bg, 
  genome = ucsc$build, min_sig = enrich_params$min_sig, threads = threads, 
  adj_method = enrich_params$adj, params = region_params$promoters, 
...
if (n_enrich > 1) {
  ...
  cor_mat <- motif_list %>% 
    ...
    to_list() %>% 
    getPwmMatches(
      fw_seq, min_score = motif_params$min_score, mc.cores = threads
    ) %>% 
    lapply(\(x) tibble(seq = unique(x$seq))) %>% 
    bind_rows(.id = "name") %>%
    distinct(name, seq) %>% 
    left_join(
      dplyr::select(enrich_res, altname, name), by = "name"
    ) %>% 
...
clust_names <- pos_res %>% 
  ...
  summarise(
    cl_name = paste(name, collapse = "/"),
    name = name[[1]], .by = cluster
  ) %>% 
  mutate(name = setNames(cl_name, name)) %>% 
  pull(name)
...
motif_list %>% 
  ...
  plotMatchPos(
    abs = motif_params$abs, type = "heatmap", cluster = length(top_pos) > 1,
    use_totals = TRUE,
...
```{r all-out}
out_path <- file.path("output", "results", "signal_summary", "shared")
if (!dir.exists(out_path)) dir.create(here::here(out_path), recursive = TRUE)
all_out <- list(
  enrich_genomic_bg = file.path(out_path, "shared_enrichment_genomic_bg.csv"),
  enrich_targets_bg = file.path(out_path, "shared_enrichment_targets_bg.csv"),
  localz_shared_regions = file.path(out_path, "shared_regions_localz.csv"),
  localz_pairwise_targets = file.path(out_path, "pairwise_localz.csv")
)
enrich_genomic_df %>% 
  mutate(gene_id = map_chr(gene_id, paste, collapse = "; ")) %>% 
  write_csv(here::here(all_out$enrich_genomic_bg))
enrich_targets_df %>% 
  mutate(gene_id = map_chr(gene_id, paste, collapse = "; ")) %>% 
  write_csv(here::here(all_out$enrich_targets_bg))
localz_df %>% write_csv(here::here(all_out$localz_shared_regions))
pairwise_df %>% write_csv(here::here(all_out$localz_pairwise_targets))
```
