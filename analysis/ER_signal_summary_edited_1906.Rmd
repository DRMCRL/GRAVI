---
title: 'ER: Summary of Detected Signal'
...
all_paths <- here::here("config", "json", "all_paths.json") %>% 
    ...
    lapply(here::here)
# all_paths <- list(
#   annotation  = here::here("output/annotations"),
#   bam         = here::here("data/bam"),
#   diff_signal = here::here("output/differential_signal"),
#   greylist    = here::here("output/greylist"),
#   json        = here::here("config/json"),
#   macs2       = here::here("output/macs2"),
#   nfr         = here::here("output/nfr"),
#   peaks       = here::here("output/peak_analysis"),
#   rmd         = here::here("analysis")
# )
...
individual_peaks %>%
  ...
  mutate(
    ...
    highlight = case_when(
      !!!imap(valid_sets, \(cols, name) if_all(all_of(cols)) ~ name)
    )
  ) %>% 
  simpleUpSet(
    sets = unname(lb), n_intersect = 30, sort_sets = NULL, min_size = 10,
    set_layers = default_set_layers(
      fill = "set", expand = c(0.25, 0),
      scale_fill_manual(
...
if (n_enrich > 1) {
  ...
  if (ncol(cor_mat) < 2) {
    message("Too few motifs with variable matches to compute correlations.")
    return(invisible(NULL))
  }
  cor_mat <- cor(cor_mat, method = "spearman")
...
if (n_pos > 0) {
  top_pos_hm <- motif_pos_res %>% 
    dplyr::filter(adj_p < motif_params$alpha) %>% 
    dplyr::slice(1:25) %>% 
    pull(altname)
  hm_motifs <- motif_list %>% dplyr::filter(altname %in% top_pos_hm)
  if (nrow(hm_motifs) > 0) {
    hm_motifs %>% 
      to_list() %>% 
      getPwmMatches(
